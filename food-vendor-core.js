...
function statementPdf(model){
 ...
 if(model.ledgerHeads){if(y>H-260)newPage();y+=24;text('Payments & adjustments',M,y,12,true);y+=12;if(model.ledgerRows?.length)table(model.ledgerHeads,model.ledgerRows,[100,100,330,120]);else {text('No payments or adjustments in this period.',M,y+18,11);y+=32}const f=model.finance||{};if(y>H-260)newPage();y+=24;text('Account summary',M,y,12,true);y+=12;table(['Particulars','Amount INR'],[['Opening balance',Number(f.opening||0).toFixed(2)],['Receipt charges',Number(f.charges||0).toFixed(2)],['Payments received','- '+Number(f.payments||0).toFixed(2)],['Adjustments',Number(f.adjustments||0).toFixed(2)],['Closing balance',Number(f.closing||0).toFixed(2)]],[330,180]);}
