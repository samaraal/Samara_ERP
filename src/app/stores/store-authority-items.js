...
function PharmacyStockPanel({stock,itemId,quantity,unit,onSelect,showSelector=true}){
  ...
  function StoreItemMaster({profile}){
    ...
    const duplicateWarning=(candidate,excludeId='')=>{const hit=similarMasterItem(candidate,excludeId);if(!hit)return false;showSamaraActionToast('error','Duplicate / similar item blocked',`${hit.item_code?hit.item_code+' · ':''}${hit.item_name} already exists under ${hit.item_category||'Stores'}. Please use or edit the existing master item. If this is genuinely different, include the distinguishing size / strength / gauge / form in the name.`);return true};
