...
Deno.serve(async (req) => {
  try {
    ...
    const isEscalationRecipientRole = (role:string) => {
      const r = String(role||"").trim().toLowerCase();
      return r === "admin" || r === "administrator" || r === "director" || r === "manager"
        || r.includes("administrator") || r.endsWith(" manager") || r.startsWith("manager ");
    };

    const subs = (activeSubs as any[]).filter((s:any) => {
      const userRole = s.user_id ? roleByProfileId.get(s.user_id) || "" : "";
      const profileRole = s.profile_id ? roleByProfileId.get(s.profile_id) || "" : "";
      return isEscalationRecipientRole(userRole) || isEscalationRecipientRole(profileRole);
    });
    ...
    for (const e of escalations as any[]) {
      ...
      const title = `${String(e.priority||"URGENT").toUpperCase()} · SAMARA CLINICAL ESCALATION`;
      const body = [
