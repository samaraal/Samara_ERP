  ...
  try {
    ...
    if (body.action === 'login_precheck' || body.action === 'login_failure' || body.action === 'login_success') {
      ...
      if (body.action === 'login_success') {
        const { data: { user: loggedIn }, error: loginError } = await caller.auth.getUser()
        if (loginError || !loggedIn) return json({ error: 'Not authenticated' }, 401)
        const { data: ownProfile } = await admin.from('profiles').select('login_id').or(`id.eq.${loggedIn.id},auth_user_id.eq.${loggedIn.id}`).maybeSingle()
        if (normalizeLoginId(ownProfile?.login_id) !== loginId) return json({ error: 'Not authorised' }, 403)
        await securityEvent(loginId, 'LOGIN_SUCCESS')
        return json({ ok: true, locked: false })
      }
      // Browser-reported failures are untrusted and must not let a caller lock another account.
      if (body.action === 'login_failure') await securityEvent(loginId, 'CLIENT_REPORTED_LOGIN_FAILURE')
    ...
    if (body.action === 'request_password_recovery') {
      ...
      const { data: target, error: targetError } = await admin.auth.admin.getUserById(authUserId)
      // Recovery never changes an address, confirms an address, or removes a ban.
      if (targetError || !target.user || !target.user.email_confirmed_at ||
          String(target.user.email || '').toLowerCase() !== email ||
          (target.user.banned_until && new Date(target.user.banned_until).getTime() > Date.now())) return json({ ok: true })
      const { data: allowed, error: throttleError } = await admin.rpc('samara_take_action_slot', { p_key: `recovery:${authUserId}`, p_limit: 3, p_seconds: 900 })
      if (throttleError || !allowed) return json({ ok: true })
      await securityEvent(normalized, 'PASSWORD_RECOVERY_REQUESTED')
      // Auth's configured redirect allowlist remains authoritative.
    ...
    const { data: callerProfile, error: callerProfileError } = await admin
      .from('duty_profiles').select('role,is_active,active').or(`id.eq.${user.id},auth_user_id.eq.${user.id}`).single()
