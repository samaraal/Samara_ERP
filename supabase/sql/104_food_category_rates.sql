...
begin
 ...
 if action='settings' then
  ...
  if unitprice is null or unitprice<0 or unitprice>1000000 or coalesce(itemname,'') not in ('Tiffin','Lunch','Dinner','Coffee/Tea') or nullif(d->>'reason','') is null then raise exception 'Select Tiffin, Lunch, Dinner or Coffee/Tea, a valid price and a reason';end if;
  ...
  if action in('save','finalise','modify') then
   ...
   i:=0;for it in select value from jsonb_array_elements(d->'items') loop
    ...
    if q>0 then
     select price into unitprice from public.fv_rates where vendor_id=o.data->>'vendor_id' and item=case when o.data->>'slot' in ('Morning Tea / Coffee','Evening Tea / Coffee') then 'Coffee/Tea' else o.data->>'slot' end and effective<=(o.data->>'date')::date order by effective desc,created_at desc limit 1;
     insert into public.fv_ledger(vendor_id,day,kind,amount,data,actor) values(o.data->>'vendor_id',(o.data->>'date')::date,'Receipt',round(q*unitprice,2),jsonb_build_object('order_id',oid,'receipt_id',eid,'slot',o.data->>'slot','item',itemname,'quantity',q,'residents',it->'residents','employees',it->'employees','unit_price',unitprice,'rate_category',case when o.data->>'slot' in ('Morning Tea / Coffee','Evening Tea / Coffee') then 'Coffee/Tea' else o.data->>'slot' end),(a->>'actor')::uuid);
    end if;i:=i+1;
   end loop;
