-- Samara Care ERP 1.3.26
-- Record and display who decided a Bill/Charge request and when.

alter table public.bill_charge_requests
  add column if not exists decision_by_name text;

alter table public.bill_charge_requests
  add column if not exists decision_by_role text;

-- Populate historical decided records where the profile can be resolved.
update public.bill_charge_requests b
set decision_by_name=coalesce(p.full_name,p.username,'Authorised user'),
    decision_by_role=p.role
from public.profiles p
where b.approval_status in ('Approved','Partially Approved','Rejected')
  and (b.decision_by_name is null or b.decision_by_name='')
  and (p.auth_user_id=b.approved_by or p.id=b.approved_by);
...
begin
  select role,coalesce(full_name,username,'Authorised user')
  into v_role,v_name
  ...
  if not found then
    raise exception 'Bill/charge request not found.';
  end if;
  ...
  if v_decision='Rejected' then
    ...
    set approval_status='Rejected',
        ...
        approved_at=now(),
        decision_date=now(),
        decision_by_name=v_name,
        decision_by_role=v_role,
        updated_at=now()
    where id=p_request_id;
...
select
  exists(
    ...
    where table_schema='public'
      ...
      and column_name='decision_by_name'
  ) as decision_by_ready,
  exists(
    select 1 from information_schema.columns
    where table_schema='public'
      and table_name='bill_charge_requests'
      and column_name='decision_date'
  ) as decision_timestamp_ready;
