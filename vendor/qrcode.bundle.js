/* Samara Care ERP offline QR generator
 * QR engine based on Kazuhiko Arase's MIT-licensed QRCode for JavaScript.
 * Bundled locally so Admission Consent generation does not require a CDN.
 */
(function(global){
  'use strict';
  var modules = {
...
},"QRCode": function(module, exports, require){
//---------------------------------------------------------------------
...
module.exports = QRCode;
  ...
  var QRCodeModel = require('QRCode');
  var Levels = require('QRErrorCorrectLevel');

  function createModel(text, level){
    var qr = new QRCodeModel(-1, Levels[level || 'M'] || Levels.M);
  ...
  function toSvg(text, options){
    options = options || {};
    var qr = createModel(text, options.errorCorrectionLevel || 'M');
    var count = qr.getModuleCount();
    var margin = Number.isFinite(options.margin) ? options.margin : 4;
    var size = Number(options.size || 180);
    ...
    for(var row=0; row<count; row++){
      for(var col=0; col<count; col++){
        if(qr.isDark(row,col)){
          var x = (col + margin) * cell;
          var y = (row + margin) * cell;
          parts.push(
            '<rect x="' + x.toFixed(3) + '" y="' + y.toFixed(3) +
            '" width="' + (cell + 0.05).toFixed(3) +
            '" height="' + (cell + 0.05).toFixed(3) + '" fill="#000"/>'
          );
        }
      }
    }
