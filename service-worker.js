const CACHE = 'samara-erp-2.9.18-pure-tamil-live-overdue-time';
const SHELL = [
  './', './index.html', './styles.css?v=2.9.18', './app.js?v=2.9.18',
...
self.addEventListener('activate', event => {
  event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== CACHE).map(k => caches.delete(k)))).then(() => self.clients.claim()).then(async()=>{
    ...
    clients.forEach(client=>client.postMessage({type:'SAMARA_UPDATE_AVAILABLE',version:'2.9.18'}));
  }));
});
