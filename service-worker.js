const CACHE = 'samara-erp-2.9.47-mobile-background-push';
const SHELL = [
  './', './index.html', './styles.css?v=2.9.47', './app.js?v=2.9.47',
  './bootstrap-error.js?v=2.8.40', './health-check.js?v=2.8.40',
  './config.js?v=2.9.47', './manifest.webmanifest?v=2.8.40',
...
self.addEventListener('activate', event => {
  event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== CACHE).map(k => caches.delete(k)))).then(() => self.clients.claim()).then(async()=>{
    ...
    clients.forEach(client=>client.postMessage({type:'SAMARA_UPDATE_AVAILABLE',version:'2.9.47'}));
  }));
});
